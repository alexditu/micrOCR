...
const char delim[]    = ".,:;`/\"+-_(){}[]<>*&^%$#@!?~/|\\=1234567890 \r\t\n";
...
static int alteration(char *word, char **array, int start_idx)
{
        ...
        for (i = 0, k = 0; i < word_len; i++)
                for (j = 0; j < (int)sizeof(alphabet); j++, k++) {
                        c[0] = alphabet[j];
...
{
        char **e1 = NULL, **e2, *e1_word, *e2_word, *res_word = word;
        int e1_rows, e2_rows;
